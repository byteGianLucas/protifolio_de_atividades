...
function ex11(data) {
    const name = 'Earth';
    // const names = prompt('Insira o nome do astro (inglês):');
...
function ex19(data) {
    const planetMasses = data.map(planet => planet.mass.massValue).sort();
    ...
    const planetAproachMass = data.find(planet => planet.mass.massValue === planetMasses[index]); 
